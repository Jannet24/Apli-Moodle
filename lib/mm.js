var MM = {
    ...
    loadSite: function(siteId){
        ...
        MM.moodleWSCall('core_enrol_get_users_courses', {userid: site.get('userid')}, function(courses) {
            ...
            for(var el in MM.config.plugins){
                ...
                if(plugin.settings.type == 'general'){
                    ...
                    plugins.push(plugin.settings);
                     ...
                     MM.coursePlugins.push(plugin.settings)
            ...
            values = {
                ...
                coursePlugins: MM.coursePlugins,
                courses: courses,
                plugins: plugins
            };
    ...
    validacion: function(e){
        ...
        var siteurl = "http://localhost:8080/moodle/";
    ...
    moodleWSCall: function(method, data, callBack, preSets, errorCallBack){
        ...
        console.log("Entro a moodleWSCall con "+ method);
    ...
    novedades: function (){
        var output3 = MM.tpl.render($('#novedades_template').html(), values);
        $(' #principal').html(output3).trigger("create");
    ...
    calendario: function(){
        MM.plugins.events.showCalendar();
    },
    calendario1: function(dia, eventos){
        var datosDia = {diaSeleccionado:dia, eventosDia: eventos};
        var output2 = MM.tpl.render($('#calendario_eventos_template').html(), datosDia);
